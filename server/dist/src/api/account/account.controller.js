...
import { ACCOUNT_STATUS, USER_ROLE, } from '../../mongo/models/account.model.js';
...
export async function updateAccount(req, res, next) {
    ...
    if (!updatedProfile) {
        logger.error(`authService - updateAccount, cannot update profile for uuid: ${uuid}`);
        throw new BadRequestError('Cannot update profile');
    }
    let account = await accountService.updateRole(accountDoc._id, USER_ROLE.user);
    account = await accountService.updateStatus(accountDoc._id, ACCOUNT_STATUS.active);
    if (!account) {
        logger.error(`authService - updateAccount, cannot up[date] account for uuid: ${uuid}`);
...
export async function getAccount(req, res, next) {
    ...
    if (!account) {
        logger.error(`authService - getAccount, cannot find account for uuid: ${uuid}`);
        throw new InternalServerError('Cannot find account');
    }
    res.status(200).json({
        success: true,
